...
LogFormatter::LogFormatter(const std::string &strFomrat) : m_strFormat(strFomrat) {
    init();
}
...
void LogFormatter::init() {
    ...
    for (size_t i = 0; i < m_strFormat.size(); ++i) {
        ...
        while (n < m_strFormat.size()) {
            if (!fmt_status
                && (!isalpha(m_strFormat[n]) && m_strFormat[n] != '{'
                    && m_strFormat[n] != '}')) {
    ...
    // s_format_items 静态map变量， key: 对应的格式标符， val：这个标符的 shared_ptr
    ...
    for (auto &i : vec) {
        if (std::get<2>(i) == 0) {
            ...
            if (it == s_format_items.end()) {
                m_formatItems.push_back(FormatItem::sptr(new StringFormatItem(
